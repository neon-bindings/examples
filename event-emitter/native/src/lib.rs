...
use neon::task::Task;
use neon::types::{JsArray, JsFunction, JsNumber, JsObject, JsString, JsUndefined, JsValue};
...
fn hello(mut cx: FunctionContext) -> JsResult<JsValue> {
    ...
    let req_body: HelloRequest =
        serde_json::from_slice(&req.body).or_else(|err| cx.throw_error(&err.to_string()))?;

    let res = HelloResponse {
        greeting: format!("Hello, {}!", req_body.name),
    };

    let res_body = serde_json::to_vec(&res).or_else(|err| cx.throw_error(&err.to_string()))?;
...
fn stringify(mut cx: FunctionContext) -> JsResult<JsString> {
    ...
    let s = serde_json::to_string(&o).or_else(|err| cx.throw_error(&err.to_string()))?;
